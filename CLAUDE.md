...
- **Commit convention** (sigil-driven — `<:code:>[(scope)]<sigil> <subject>`; the sigil `= ~ ^ ! %` is the version signal and the gitmoji decides nothing; enforced/consumed by glyph) → [`CONTRIBUTING.md`](CONTRIBUTING.md) (single source of truth).
