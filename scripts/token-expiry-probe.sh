...
# expiry reminders (homebrew-tap-token-expiry-reminder.yml, and the shape
# fleet-sync-pat-expiry-reminder.yml has) each did this inline, which made the
# decision unreachable from any test: the branch that files the reminder only
# runs when a real token is really dying, so the interesting paths shipped
# unexecuted. t-j72g is that gap — the homebrew reminder's 401 liveness branch
# has been armed since 2026-07-20 and has never once run.
#
...
# point (t-6bhz reissues it with an expiry to close that hole).
set -uo pipefail
...
case "$code" in
  401)
    emit "due=true"
    emit "kind=dead"
    emit "reason=probe got 401 — the token is dead (revoked or expired); cask pushes are failing NOW"
    exit 0
    ;;
  200) ;;
  *)
