...
# reminders. Every case here is a path that, inline in the workflow, could only
# be reached by a real token really dying: the 401 liveness branch has been
# armed in homebrew-tap-token-expiry-reminder.yml since 2026-07-20 and has never
# executed once (t-j72g). Runs the REAL script; self-test.yml invokes this.
...
' 'due=true
kind=dead
reason=probe got 401 — the token is dead (revoked or expired); cask pushes are failing NOW'
...
# The live state of HOMEBREW_TAP_TOKEN as of 2026-08-02: issued non-expiring, so
# the calendar path cannot arm at all. The reason line has to SAY that — this is
# the observation t-6bhz acts on.
